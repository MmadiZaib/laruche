...
class GiftRepository extends ServiceEntityRepository
{
    ...
    public function getStats()
    {
        return $this->createQueryBuilder('g')
            ->select('count(g.id) as totalGift, 
                count(DISTINCT g.countryCode) as totalCountries,
                MIN(g.price) as minGiftPrice, 
                MAX(g.price) as maxGiftPrice, 
                AVG(g.price) as averageGiftPrice')
            ->getQuery()
            ->getResult()
        ;
    }
